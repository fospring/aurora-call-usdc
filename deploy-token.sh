...
code=$(cat out/tUSDC.sol/tUSDC.json | jq '.bytecode .object' | cut -c 4- | rev | cut -c 2- | rev)
cat out/tUSDC.sol/tUSDC.json | jq '.abi' > out/tUSDC.sol/tUSDC.abi
aurora-cli --network testnet --near-key-path $near_key_path deploy --code $code --abi-path out/tUSDC.sol/tUSDC.abi --args '{}' --aurora-secret-key $aurora_key
# Contract has been deployed to address: 0xbcf48399b098d09d5588cf64778db338f55f6078 successfully